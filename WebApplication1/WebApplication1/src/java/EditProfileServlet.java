...
public class EditProfileServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if(request.getParameter("role").toString().equalsIgnoreCase("Staff") || request.getParameter("role").toString().equalsIgnoreCase("Owner Boat"))
            {
                Staff s = StaffDao.getStaffById(request.getParameter("id").toString());
...
//                request.setAttribute("s", r);
                ...
                request.setAttribute("license", s.getLicense());
                request.setAttribute("status", s.getStatus());
                if("Staff".equalsIgnoreCase(s.getRole()))
                    request.getRequestDispatcher("EditProfile.jsp").forward(request, response);
                else if(request.getParameter("action") != null)
                {
                    request.getRequestDispatcher("addNewBoat.jsp").forward(request, response);
                }
                else
                    request.getRequestDispatcher("EditProfile.jsp").forward(request, response);
            }
            else if(request.getParameter("role").toString().equalsIgnoreCase("ADMIN"))
            {
            ...
            {
                Customer status = CustomerDao.getCustomerById(request.getParameter("phone"));
